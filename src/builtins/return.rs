...
mod tests
{
    ...
    fn test_return_builtin_function_returns()
    {
        ...
        interp.increase_fun_count();
        let status = main(&[], args.as_slice(), &mut interp, &mut exec, &mut env, &mut settings);
        interp.decrease_fun_count();
        exec.clear_files();
        assert_eq!(0, status);
        assert!(interp.has_return());
    ...
    #[sealed_test(before=setup(), after=teardown())]
    fn test_return_builtin_function_returns_with_other_status()
    {
        let mut exec = Executor::new();
        let mut interp = Interpreter::new();
        let mut env = Environment::new();
    ...
    #[sealed_test(before=setup(), after=teardown())]
    fn test_return_builtin_function_complains_on_is_not_in_function()
    {
        let mut exec = Executor::new();
        let mut interp = Interpreter::new();
        let mut env = Environment::new();
        ...
        assert_eq!(false, interp.exec_redirect_flag());
        assert_eq!(String::new(), read_file("stdout.txt"));
        assert_eq!(String::from("Isn't in function\n"), read_file("stderr.txt"));
        assert_eq!(String::new(), read_file("stderr2.txt"));
    }
    
    #[sealed_test(before=setup(), after=teardown())]
    fn test_return_builtin_function_complains_on_too_many_arguments()
    {
        let mut exec = Executor::new();
        let mut interp = Interpreter::new();
        let mut env = Environment::new();
    ...
    #[sealed_test(before=setup(), after=teardown())]
    fn test_return_builtin_function_complains_on_invalid_number()
    {
        let mut exec = Executor::new();
        let mut interp = Interpreter::new();
        let mut env = Environment::new();
